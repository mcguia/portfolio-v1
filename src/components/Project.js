...
const Project = ({ data }) => {
  const { title, type, startDate, endDate, gitlab, external, featuredInfo, technologies, featuredImage, mainContent } = data;
    ...
    <ProjectContainer id="project">
      <ProjectHeader>
        <ProjectRow>
          ...
          <ProjectColumn>
            <Img fluid={featuredImage.fluid} imgStyle={{position: "relative"}}/>
          </ProjectColumn>
        </ProjectRow>
      </ProjectHeader>
      ...
      <ProjectInfo>
        <InfoRow>
          ...
          <RightColumn>
            ...
            <InfoText>{months[date1.getMonth()] + " " + date1.getFullYear()}
            {endDate && "–" + months[date2.getMonth()] + " " + date2.getFullYear()}</InfoText>
            <Stripe />
            {gitlab && <IconLink
                href={gitlab}
                target="_blank"
